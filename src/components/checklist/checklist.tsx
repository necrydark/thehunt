...
import { weaponSubmission } from "@/app/schemas/clip";
import {
  itemRarity,
  itemTypes,
  rarityColors,
...
export default function Checklist() {
  ...
  const handleSubmit = (data: WeaponSubmissionValues) => {
    submitMutation.mutate({
      itemId: data.itemId,
      twitchClipUrl: data.twitchClipLink,
    });
  ...
  const mergedItems = useMemo(() => {
    ...
    return allItems.map((item) => ({
      ...item,
      // Default to 'not_submitted' if no progress found (e.g., for non-logged-in users or new items)
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={category} onValueChange={setCategory}>
              ...
              <SelectContent>
                ...
                {/* Use 'Com' or 'com' as per your map */}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {mergedItems &&
          mergedItems.map((weapon) => (
            <Card
              key={weapon.id}
              className="bg-black/20 border-white/10 text-white relative"
            >
              <CardHeader>
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      ...
                      onClick={() => form.setValue("itemId", weapon.id)}
                    >
                      {weapon.userItemStatus === "approved"
                        ? "Completed"
                        : weapon.userItemStatus === "pending"
