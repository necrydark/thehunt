"use server"
import { clipSchema, submissionSchema } from "@/app/schemas/clip";
...
if(!session?.user.id) {
  ...
  try {
    ...
    return { success: true, message: 'Clip submitted successfully! Awaiting admin review.'
  }
...
export async function reviewSubmission(data: z.infer<typeof submissionSchema>) {
  const session = await auth.api.getSession({
    headers: await headers() // you need to pass the headers object.
})
